"""Scraper for Hawks & Reed Performing Arts Center (Greenfield)."""
...
BASE_URL = "https://www.hawksandreed.com"
EVENTS_URL = f"{BASE_URL}/events"


class HawksReedScraper(BaseScraper):
    name = "hawks-reed"
    town = "Greenfield"
    ...
    def _fetch(self) -> list[Event]:
        ...
        for item in soup.select(".event, article, .event-item, .tribe-event"):
            try:
                ...
                if any(w in title_lower for w in ["comedy", "stand-up", "standup", "comedian"]):
                    category = "comedy"
                elif any(w in title_lower for w in ["film", "movie", "cinema", "screen"]):
                    category = "film"
                elif any(w in title_lower for w in ["dance", "theater", "theatre", "play", "performance"]):  # noqa: E501
                    category = "arts"
                else:
                    category = "music"
                ...
                events.append(Event(
                    ...
                    date=date,
                    time=time,
                    venue="Hawks & Reed Performing Arts Center",
                    address="289 Main St, Greenfield, MA 01301",
                    town=self.town,
                    description=desc,
                    url=href,
